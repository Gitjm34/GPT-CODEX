...
RUN_ID=${RUN_ID:-}
IDS_ENDPOINT=${IDS_ENDPOINT:-}
IDS_TIMEOUT=${IDS_TIMEOUT:-0.8}

mkdir -p "$LOG_DIR"
export NS3_PATH

cmd=(
  "$PYTHON_BIN" "$MW_HOME/udp_mw_ns3.py"
  --ns3-path "$NS3_PATH"
...
if [[ -n "$IDS_ENDPOINT" ]]; then
  cmd+=( --ids-endpoint "$IDS_ENDPOINT" )
fi
