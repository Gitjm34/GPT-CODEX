...
9. Attack Automation (attackctl.py)
   1. DoS flood (20 s, 800 B payload, 30 packets/s, target ID 3 → default port set)
   ```bash
   python3 ~/mw_ns3/attackctl.py dos 20 --payload-bytes 800 --pps 30 --target-port 14640 --tag link3

2. Heartbeat drop (15 s, 60 % loss, loopback interface)
   ```bash
