...
### 4. Convert to CSV (optional features for ML)
```bash
...
* **GET** `/obs/latest?k=N`
* **GET** `/obs/extra/latest?k=N`
* **GET** `/obs/seq?since_seq=...&limit=...&run_id=...`
...
### 3. ROS Core + MAVROS + altitude mirroring
```bash
...
```bash
export RUN_ID="run_test001"
export IDS_EXTRA_ENDPOINT="[http://127.0.0.1:8080/ingest_extra](http://127.0.0.1:8080/ingest_extra)"
source /opt/ros/noetic/setup.bash
...
### 7. Verification
```bash
curl -s "[http://127.0.0.1:8080/obs/latest?k=3](http://127.0.0.1:8080/obs/latest?k=3)" | python3 -m json.tool
curl -s "[http://127.0.0.1:8080/obs/seq?since_seq=100&limit=10](http://127.0.0.1:8080/obs/seq?since_seq=100&limit=10)" | python3 -m json.tool
```
...
* A 2s "blackout" can be injected by writing `0 0 333` to `positions.txt`, yielding `loss_pct≈100%` and near-zero downlink bytes for that window.
...
## Utility & Helper Scripts (copy each to the indicated path)
